...
def detect(fram,videoname,imagename):

	# resize the frame to have a width of 600 pixels (while
	# maintaining the aspect ratio), and then grab the image
	# dimensions
	frame = imutils.resize(fram, width=600)
	(h, w) = frame.shape[:2]
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			if name =="presenter":
				createFolder("presenters/" + str(videoname))
				cv2.imwrite("presenters/" + str(videoname) + "/%s____%.3f.jpg" %(name,(proba*100)), frame)
	return  frame
